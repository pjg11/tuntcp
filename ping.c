#define _POSIX_C_SOURCE 199309L
#define IFNAMSIZ 16

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/time.h>
#include <arpa/inet.h>
#include <time.h>
#include "tcpip.h"

int main(int argc, char *argv[]) {

	if (argc < 3) {
		printf("Usage: ./ping <num> <dest_ip>");
		return 1;
	}

	int tun = openTun("tun0");

	char addr[INET_ADDRSTRLEN];

	size_t size = sizeof(struct ipv4) + sizeof(struct icmpecho);
	char *packet = malloc(size);

	struct ipv4 *ip = IPV4(sizeof(struct icmpecho), 1, argv[2]);
	bytes(ip, packet, sizeof(*ip));

	printf("PING %s %d bytes of data.\n", inet_ntop(AF_INET, &(ip->dst), addr, INET_ADDRSTRLEN), ntohs(ip->len));
	
	struct timeval tv;
    tv.tv_sec  = 500 / 1000;
    tv.tv_usec = (500 % 1000) * 1000;
    select (0, NULL, NULL, NULL, &tv);
	
	for ( int i = 0; i < atoi(argv[1]); i++ ) {
		
		struct icmpecho *e = ICMPEcho(i+1);
		bytes(ip, packet, sizeof(*ip));
		bytes(e, packet + sizeof(*ip), sizeof(*e));
		
		char buffer[size];

		struct timespec start, end;

		clock_gettime(CLOCK_REALTIME, &start);
		write(tun, packet, size);
		int len = read(tun, buffer, sizeof(buffer));

		clock_gettime(CLOCK_REALTIME, &end);

		struct icmpecho *re = calloc(1, sizeof(struct icmpecho));
		struct ipv4 *ri = calloc(1, sizeof(struct ipv4));
		
		memcpy(ri, packet, sizeof(*ri));
		memcpy(re, (void *)packet + sizeof(*ri), sizeof(*re));

    	double elapsed = (end.tv_sec - start.tv_sec) + ((end.tv_nsec - start.tv_nsec) / 1000000.0);
		printf("%d bytes from %s: icmp_seq=%d ttl=%d time=%.3f ms\n", len, "8.8.8.8", ntohs(re->seq), ri->ttl, elapsed);
	
		sleep(1);
	}

	free(ip);
	free(packet);
	close(tun);
	return 0;
}